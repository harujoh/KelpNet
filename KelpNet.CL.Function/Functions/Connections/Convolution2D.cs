...
namespace KelpNet.CL
{
    ...
    {
        public static NdArray<T>[] Convolution2D<T>(NdArray<T> x, NdArray<T> weight, NdArray<T> bias = null, int strideX = 1, int strideY = 1, int padX = 0, int padY = 0, ICompressibleActivation<T> activation = null, bool gpuEnable = false) where T : unmanaged, IComparable<T>
        {
            return new Convolution2D<T>(weight.Shape[0], weight.Shape[1], new[] { weight.Shape[2], weight.Shape[3] }, new[] { strideX, strideY }, new[] { padX, padY }, bias != null, weight.Data, bias?.Data, activation, gpuEnable: gpuEnable).Forward(x);
        }
    }
#endif
